...
<br>                              <table border="3" class="table table-striped responsive-utilities jambo_table">
									<thead>
										<tr>
											<th>No</th>
											<th>Pelajaran</th>
...
if(empty($data)){
	...
	foreach ($data as $data) {
		...
		<tr>
			<td>".$no."</td>
			<td><a href='".$url."/katilu/?ajaran=".$data['id_semester']."&siswa=".$data['id_siswa']."&kelas=".$data['id_kelas']."&mapel=".$data['id_mapel']."'>
			".$data['nama_mapel']."</a></td>
			<td><a href='".$url."/katilu/?ajaran=".$data['id_semester']."&siswa=".$data['id_siswa']."&kelas=".$data['id_kelas']."&mapel=".$data['id_mapel']."'>
			".$data['tingkat']." ".$data['jurusan']." ".$data['kelas']."</a></td>
		</tr>
		";
		$no++;
	}
